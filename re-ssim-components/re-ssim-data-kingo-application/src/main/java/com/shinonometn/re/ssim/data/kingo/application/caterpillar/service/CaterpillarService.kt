...
//                    .count()
     ...
     * @return created task
     */
    fun createTask(termCode: String): CaptureTask {
    ...
    fun resumeTask(captureTask: CaptureTask): CaptureTaskDetails? {
        ...
        val spiderStatus = getTaskDetails(captureTask).capturing
                ?: throw BusinessException("task_have_not_initialized")
    ...
    fun startByTaskIdAndSettings(taskId: Int, caterpillarSetting: CaterpillarSetting): CaptureTaskDetails {
        ...
        if (captureTaskDetails.capturing != null) throw BusinessException("task_thread_exists")

        val captureTask = captureTaskDetails.taskInfo ?: throw Exception("unknown_exception")
    ...
    fun bundleData(task: CaptureTask, deleteOld: Boolean): File {
        ...
        getTaskDetails(task).capturing?.run {
    ...
    private fun getTaskDetails(captureTask: CaptureTask): CaptureTaskDetails {
        ...
        captureTaskDetails.capturing = spiderMonitor.getSpiderStatus()[captureTask.id.toString()]
        return captureTaskDetails
    }
